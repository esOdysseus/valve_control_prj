...
// setter
bool CVctrlCMD::set_when(time_t when) {
    try {
        when_tm = time_pkg::CTime::convert<tm>(when);
        assert( (when_double = time_pkg::CTime::convert<double>(when)) > 0.0 );
        return true;
    }
    catch( const std::exception &e ) {
...
bool CVctrlCMD::set_how(const std::string &method) {
    if ( method == VALVE_HOW_OPEN || method == VALVE_HOW_CLOSE ) {
        how = method;
        return true;
    }
...
*/
CVctrlCMD::E_WEEK CVctrlCMD::get_week(struct tm &time) {
